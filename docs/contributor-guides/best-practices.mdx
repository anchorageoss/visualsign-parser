...
Ensure all changes and refactors are tested thoroughly. Use `cargo-llvm-cov` for test coverage:
...
# Generate report for a specific package
cargo llvm-cov --package visualsign-solana --lib --open

# For CI: generate lcov format or fail under threshold
cargo llvm-cov --workspace --lcov --output-path lcov.info
cargo llvm-cov --workspace --fail-under-lines 70
```

Reports are generated in `target/llvm-cov/html/`. Use coverage to identify untested code paths and prioritize adding test fixtures for uncovered instruction types.
