...
description: Verify a preset against a real transaction with parser_cli, and add the fixture pair CI will run
---
...
```bash
parser_cli decode --chain <chain> -t <your_transaction_hex> --output human
```

For example, with an Ethereum transaction:

```bash
parser_cli decode --chain ethereum -t 0x02f9... --output human
```
...
```bash
parser_cli decode --chain <chain> -t <your_transaction_hex> --output human --condensed-only
```
...
```bash
parser_cli decode --chain <chain> -t <your_transaction_hex> --output json
```

You can extract specific fields with `jq`:

```bash
parser_cli decode --chain ethereum -t <tx_hex> --output json | jq '.Fields'
```
