...
The only shared file a preset PR touches is that module list, and only to append a line. No existing decoder, no dispatch table, no core type changes. Reviewers read each preset PR in isolation. CI sees no ripple.
...
If you're **integrating VisualSign into a wallet**, composability is built into the data layer. The Ethereum side models token resolution as a `LayeredRegistry<T>`: a request-scoped registry layered over the compiled-in defaults, so wallet-provided overrides can take precedence at lookup time without forking VisualSign. The seam exists today; wiring wallet-supplied token metadata into that request layer is still a TODO (`create_layered_registry` in `visualsign-ethereum/src/lib.rs` currently uses defaults only). The Solana side composes through fixed IDL + preset layers; wallet-side overlays are the natural next step on both chains.
...
If you're **writing tests**, surfpool (PR #202) composes real Solana mainnet state into the local harness. Tests run against a local mainnet fork — backed by live RPC (Helius, a custom `SOLANA_RPC_URL`, or public mainnet-beta), so they need network access but never broadcast to a live cluster. Forking real chain data catches drift between IDL assumptions and on-chain reality.

## The seams are the product

"Composable by design" is a phrase. The work is in keeping the seams clean as surface area grows. Build-time discovery means the dispatch registry can't drift out of sync with the directories on disk — the one list a contributor still maintains by hand is the `mod.rs` module declarations, and a missing line fails the build loudly rather than silently. Field builders mean two contributors emitting the same content produce identical bytes. And a shared IDL parser between the raw instruction and the preset means every preset decodes through the same typed view rather than hand-rolling byte parsing.
