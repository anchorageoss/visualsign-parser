...
description: Decode Solana instructions from an Anchor IDL, the path most Solana presets take
---
...
fn parse_jupiter_instruction(data: &[u8]) -> Result<(), VisualSignError> {
    let program_id = "JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4";
    let idl = ProgramType::from_program_id(program_id)
        .and_then(|pt| decode_idl_data(pt.idl_json()).ok())
        .ok_or_else(|| VisualSignError::DecodeError("IDL not found".into()))?;

    let parsed = parse_instruction_with_idl(data, program_id, &idl)
        .map_err(|e| VisualSignError::DecodeError(format!("Failed to parse instruction: {e}")))?;
...
fn parse_transfer(data: &[u8]) -> Result<MyInstruction, MyError> {
    ...
    let amount = u64::from_le_bytes(
        data[2..10].try_into()
            .map_err(|_| MyError::DataTooShort("transfer amount"))?,
    );
