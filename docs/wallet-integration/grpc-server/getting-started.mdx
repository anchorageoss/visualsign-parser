...
signature is chosen at startup, not per request. We recommend passing exactly one
of these flags explicitly, rather than relying on the default described below:
