...
Built-in token metadata is preloaded only for `ETHEREUM_MAINNET`, `POLYGON_MAINNET`, `ARBITRUM_MAINNET`, `OPTIMISM_MAINNET`, and `BASE_MAINNET`. On other recognized chains, token symbol and decimal resolution is limited to the built-in registry — `chain_metadata` currently only accepts `network_id` and `abi_mappings`; wallet-supplied token metadata is not yet supported. Always pass the **actual** `network_id` for the chain you're parsing; the value overrides the transaction's chain ID and reusing an unrelated one will mislabel the Network field.
