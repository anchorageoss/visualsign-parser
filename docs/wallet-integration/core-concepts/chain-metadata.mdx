        ...
        Metadata: &pb.ChainMetadata_Near{
            Near: &pb.NearMetadata{
                ...
                TokenMappings: map[string]*pb.TokenMetadataEntry{
                    "nep141:a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48.factory.bridge.near": &pb.TokenMetadataEntry{
                        Value: `{"symbol":"USDC.e","decimals":6}`,
                    },
                },
            },
        },
...
The signature covers `value`'s bytes verbatim (the same convention as `Abi.value`/`Idl.value`), bound to the asset id so it cannot be replayed under a different one. An entry with no signature is still accepted; one with a signature that fails to verify is rejected outright, not silently downgraded to unsigned. See [`visualsign::signing`](https://github.com/anchorageoss/visualsign-parser/blob/main/src/visualsign/src/signing.rs) for the exact prehash construction.
