...
`abi_mappings` keys are 0x-prefixed contract addresses. The parser does not normalize them, so a contract registered under one casing won't match a transaction targeting the same address with a different casing. Pick one convention — all lowercase or EIP-55 checksummed — and apply it consistently.
