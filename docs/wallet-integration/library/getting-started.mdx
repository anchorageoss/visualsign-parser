...
### Direct ABI registry (Ethereum)

If you use the Ethereum parser directly (without the top-level registry), you can build an `AbiRegistry` yourself and pass it to `convert_from_string_with_abi`. This is useful when you already have ABIs loaded from files or a database:
...
// Build an ABI registry from your own sources
let mut abi_registry = AbiRegistry::new();
let abi_json = std::fs::read_to_string("/path/to/abi.json")?;
register_embedded_abi(&mut abi_registry, "0x1234...abcd", &abi_json)?;

// Map address to the ABI for automatic resolution during parsing
let address = "0x1234...abcd".parse()?;
abi_registry.map_address(1, address, "0x1234...abcd");

// Parse with the explicit ABI registry
let converter = EthereumVisualSignConverter::new();
let payload = converter.convert_from_string_with_abi(
    raw_tx,
