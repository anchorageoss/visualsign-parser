...
The `network_id` controls which token registry the parser uses when resolving contract addresses to symbols. If you omit it, the parser uses the chain ID embedded in the transaction (present in EIP-1559 and EIP-155 legacy transactions). It only defaults to mainnet when no chain ID is available from either source, which is only possible for pre-EIP-155 legacy transactions that carry no chain ID. Always set `network_id` explicitly so token resolution is correct regardless of transaction type.
