...
{/*
TODO: Publish a public Docker image and add Docker deployment instructions.
The K8s example below uses a placeholder image that doesn't exist yet.
*/}
...
# Run the parser (starts all required components)
make parser
```
...
```bash
# gRPC health check
grpcurl -plaintext localhost:44020 health.HealthCheckService/AppHealth
```

This returns `{"code": 200}` when healthy.
