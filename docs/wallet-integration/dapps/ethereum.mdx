...
If your dApp's contracts aren't ERC-20, ERC-721, Permit2, or a Uniswap router, the parser cannot decode their calldata without help. To users, that means a 4-byte selector and an opaque hex blob in the signing screen — exactly the kind of "approve this thing you can't read" UX that VisualSign exists to eliminate.
...
- **Use canonical interfaces wherever possible.** Calls to `transfer(address,uint256)`, `approve(address,uint256)`, ERC-721 transfers, Permit2, and Uniswap routers are decoded automatically — no ABI required. If your contract's functions can be expressed in terms of standard interfaces, do so.
