...
| Trust model | Code reviewed by the OSS maintainers, runs in the wallet's parsing environment (attestation-verified TEE in production, library or gRPC server in other deployment modes) | Wallet has to decide whether to trust the ABI source |
...
- **Hash**: SHA-256 over a domain-separated preimage, not the raw ABI JSON alone. The preimage length-prefixes and concatenates four fields — the ASCII domain tag `visualsign-metadata-v1`, the chain tag `ethereum`, a 28-byte scope (8-byte big-endian chain ID followed by the 20-byte contract address), and the ABI JSON bytes — each prefixed with its own 8-byte little-endian length. Binding the chain ID and contract address into the hash stops a signature minted for one contract from being replayed against another.
...
Sign the domain-separated prehash, not a plain hash of the ABI JSON. In Node.js:

```javascript
import { createHash } from "node:crypto";
import { secp256k1 } from "@noble/curves/secp256k1";

const DOMAIN = "visualsign-metadata-v1"; // version tag, do not change
const CHAIN_TAG = "ethereum";

// Prefixes `field` with its own length as an 8-byte little-endian u64.
...
const chainId = 1n; // e.g. Ethereum mainnet; must match the chain the ABI is registered on
const contractAddress = "0x1234567890123456789012345678901234567890"; // replace with your real contract address

// scope = 8-byte big-endian chain ID || 20-byte contract address
const chainIdBytes = Buffer.from(chainId.toString(16).padStart(16, "0"), "hex");
const addressBytes = Buffer.from(contractAddress.slice(2), "hex");
const scope = Buffer.concat([chainIdBytes, addressBytes]);

const preimage = Buffer.concat([
  lengthPrefixed(Buffer.from(DOMAIN, "ascii")),
  lengthPrefixed(Buffer.from(CHAIN_TAG, "ascii")),
  lengthPrefixed(scope),
  lengthPrefixed(Buffer.from(abiJson, "utf8")),
]);
