---
title: Parser CLI
description: Decode a transaction locally and preview its visualization before opening a PR
---
...
```bash
parser_cli decode --chain <chain> -t <transaction_hex> --output <format>
```
...
```bash
parser_cli decode --chain ethereum -t <transaction_hex> --output human --condensed-only
```
...
parser_cli decode --chain ethereum -t <your_tx_hex> --output json > result.json
...
parser_cli decode --chain ethereum -t <tx_hex> --output json | jq '.'

# Focus on specific fields
parser_cli decode --chain ethereum -t <tx_hex> --output json | jq '.Fields[-1]'
```
...
parser_cli decode --chain ethereum -t <tx_hex> --output human --condensed-only

# Verify character limits and formatting
parser_cli decode --chain ethereum -t <tx_hex> --output json | jq '.PreviewLayout.Condensed'
```
...
parser_cli decode --chain ethereum -t <tx> --output json | jq -r '.. | .Title? // empty'

# Find all addresses
parser_cli decode --chain ethereum -t <tx> --output json | jq -r '.. | .Address? // empty'
```
...
The CLI enables `allow_signed_transactions` by default, which allows parsing already-signed transactions by extracting the unsigned portion. This is useful for development and debugging — you can paste transactions directly from block explorers.
...
parser_cli decode --chain ethereum -t 0x02f8b00181...signature_bytes...
