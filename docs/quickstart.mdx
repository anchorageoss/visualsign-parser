...
```bash
./target/release/parser_cli decode --chain ethereum -t <your_transaction_hex> --output human
```
...
```bash
./target/release/parser_cli decode --chain ethereum -t 0xf86c808504a817c800825208943535353535353535353535353535353535353535880de0b6b3a76400008025a0 --output human
```
...
```bash
./target/release/parser_cli decode --chain solana -t AgAAAAAAAA... --output human
```
...
```bash
./target/release/parser_cli decode --chain ethereum -t <tx_hex> --output json
```
...
./target/release/parser_cli decode --chain ethereum -t <tx_hex> --output json | jq -r '.Fields[].Label'

# Get the transaction title
./target/release/parser_cli decode --chain ethereum -t <tx_hex> --output json | jq -r '.Title'
```
