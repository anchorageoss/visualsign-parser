...
use qos_core::{
    ...
    protocol::{ENCLAVE_APP_SOCKET_CLIENT_TIMEOUT_SECS, msg::ProtocolMsg},
};
    ...
    Req: Message,
{
    let (replier, receiver) =
        sync_channel::<Result<Resp, tonic::Status>>(SOCKET_MESSAGE_QUEUE_BUFFER_SIZE);

    response_channel
        .send(SocketMessage { request, replier })
        .map_err(|e| {
