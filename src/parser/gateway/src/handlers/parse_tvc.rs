...
pub async fn parse_handler_tvc(
    ...
    let offers = build_canonical_offers(&config);
...
/// 402 PaymentRequired with the canonical x402 v2 body listing every
/// configured price tag. Translates our internal network/asset names to
/// the CAIP-2 form payai's TS client expects, and includes payai's known
/// fee-payer in `extra` so the buyer can build a tx with payai as
/// fee-payer (the only fee-payer the facilitator will co-sign at /settle).
///
/// Parse-chain and settlement-chain are independent: the buyer may pay
/// on any configured tag regardless of which chain they're asking the
/// parser to decode. An empty `accepts` only happens when the gateway
/// was started with no price tags (operator misconfiguration), which we
/// surface as 500 — the gateway is unusable in that state.
fn payment_required(config: &X402Config) -> (StatusCode, HeaderMap, Json<TurnkeyResponseWrapper>) {
    let accepts = build_canonical_offers(config);
    if accepts.is_empty() {
        return error(
            StatusCode::INTERNAL_SERVER_ERROR,
            "gateway has no x402 price tags configured",
        );
    }
...
/// Build the canonical `accepts[]` array the gateway is willing to honor.
/// The same logic powers the 402 emission AND the per-request validation
/// gate in `validate_accepted_is_offered` — sharing the construction is
/// how we make sure the buyer can never echo back an offer the gateway
/// didn't actually make. Parse-chain and settlement-chain are independent,
/// so every configured tag is offered for every request.
fn build_canonical_offers(config: &X402Config) -> Vec<serde_json::Value> {
    config
        .price_tags
        .iter()
        .map(|t| {
