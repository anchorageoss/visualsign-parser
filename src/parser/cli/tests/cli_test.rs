...
fn test_cli_with_fixtures() {
    ...
    for input_file in test_cases {
        ...
        // For JSON fixtures, compare parsed values to avoid key-ordering
        ...
        if test_name.ends_with("-json") {
            let expected_json: serde_json::Value = serde_json::from_str(expected)
                .unwrap_or_else(|e| panic!("Invalid JSON in expected file '{test_name}': {e}"));
            let actual_json: serde_json::Value = serde_json::from_str(actual)
                .unwrap_or_else(|e| panic!("Invalid JSON in CLI output '{test_name}': {e}"));

            assert_eq!(
                expected_json, actual_json,
                "Test case '{test_name}' failed: JSON values differ.\nExpected:\n{expected}\nActual:\n{actual}"
            );
