//! Command line interface for parser host.
...
impl HostOptions {
    fn new(args: &mut Vec<String>) -> Self {
        let parsed = OptionsParser::<HostParser>::parse(args)
            .expect("provided invalid CLI args for Parser host");
    ...
    fn vsock_to_host_flag(&self) -> u8 {
        let include = self
            ...
            .as_ref()
            .map(|s| s.parse())
            .map(|r| r.expect("could not parse `--vsock-to-host`. Valid args are true or false"))
            .unwrap_or(false);
