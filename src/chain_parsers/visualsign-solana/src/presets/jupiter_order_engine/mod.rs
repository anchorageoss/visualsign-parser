//! Jupiter Order Engine preset implementation for Solana
...
use config::JupiterOrderEngineConfig;
use solana_parser::{
...
pub(crate) const JUPITER_ORDER_ENGINE_PROGRAM_ID: &str =
    "61DFfeTKM7trxYcPQCM78bJ794ddZprZpAwAnLiwTpYH";

const JUPITER_ORDER_ENGINE_IDL_JSON: &str = include_str!("jupiter_order_engine.json");

static JUPITER_ORDER_ENGINE_CONFIG: JupiterOrderEngineConfig = JupiterOrderEngineConfig;

pub struct JupiterOrderEngineVisualizer;

impl InstructionVisualizer for JupiterOrderEngineVisualizer {
    fn visualize_tx_commands(
        ...
        let parsed = parse_jupiter_order_engine_instruction(data, &accounts);

        let (title, condensed_fields, mut expanded_fields) = match parsed {
            Ok(parsed) => build_parsed_fields(&parsed, &program_id)?,
            Err(e) => {
                tracing::warn!("Failed to parse Jupiter Order Engine instruction with IDL: {e}");
    ...
    fn get_config(&self) -> Option<&dyn SolanaIntegrationConfig> {
        Some(&JUPITER_ORDER_ENGINE_CONFIG)
    }

    fn kind(&self) -> VisualizerKind {
        VisualizerKind::Dex("Jupiter Order Engine")
    }
}

fn get_jupiter_order_engine_idl() -> Option<&'static Idl> {
    static IDL: OnceLock<Option<Idl>> = OnceLock::new();
    IDL.get_or_init(|| decode_idl_data(JUPITER_ORDER_ENGINE_IDL_JSON).ok())
        .as_ref()
}

fn parse_jupiter_order_engine_instruction(
    data: &[u8],
    accounts: &[AccountMeta],
) -> Result<JupiterOrderEngineParsedInstruction, Box<dyn std::error::Error>> {
    if data.len() < 8 {
        return Err("Invalid instruction data length".into());
    }

    let idl = get_jupiter_order_engine_idl().ok_or("Jupiter Order Engine IDL not available")?;
    let parsed = parse_instruction_with_idl(data, JUPITER_ORDER_ENGINE_PROGRAM_ID, idl)?;

    let (named_accounts, extra_accounts) = build_named_accounts(data, idl, accounts);

    Ok(JupiterOrderEngineParsedInstruction {
        parsed,
        named_accounts,
        extra_accounts,
    })
...
struct JupiterOrderEngineParsedInstruction {
...
fn build_parsed_fields(
    instruction: &JupiterOrderEngineParsedInstruction,
    program_id: &str,
) -> Result<
    (
        String,
...
> {
    ...
    let title = format!("Jupiter Order Engine: {instruction_name}");

    let mut condensed_fields = vec![];
    let mut expanded_fields = vec![];

    condensed_fields.push(create_text_field("Program", "Jupiter Order Engine")?);
...
fn build_fallback_fields(
    ...
    VisualSignError,
> {
    let title = "Jupiter Order Engine: Unknown Instruction".to_string();
...
mod tests {
    ...
    #[test]
    fn test_jupiter_order_engine_idl_loads() {
        let idl = get_jupiter_order_engine_idl();
        assert!(
            idl.is_some(),
            "Jupiter Order Engine IDL should load successfully"
        );
        let idl = idl.unwrap();
    ...
    #[test]
    fn test_jupiter_order_engine_idl_has_discriminators() {
        let idl = get_jupiter_order_engine_idl().unwrap();
    ...
    fn test_unknown_discriminator_returns_error() {
        ...
        let accounts = vec![];
        let result = parse_jupiter_order_engine_instruction(&garbage_data, &accounts);
    ...
    fn test_short_data_returns_error() {
        ...
        let accounts = vec![];
        let result = parse_jupiter_order_engine_instruction(&short_data, &accounts);
