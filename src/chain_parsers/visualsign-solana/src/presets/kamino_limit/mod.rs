//! Kamino Limit Orders preset implementation for Solana
...
pub(crate) const KAMINO_LIMIT_PROGRAM_ID: &str = "LiMoM9rMhrdYrfzUCxQppvxCSG1FcrUK9G8uLq4A1GF";

const KAMINO_LIMIT_DISPLAY_NAME: &str = "Kamino Limit Orders";

const KAMINO_LIMIT_IDL_JSON: &str = include_str!("kamino_limit.json");

static KAMINO_LIMIT_CONFIG: KaminoLimitConfig = KaminoLimitConfig;

pub struct KaminoLimitVisualizer;

impl InstructionVisualizer for KaminoLimitVisualizer {
    fn visualize_tx_commands(
        ...
        let parsed_result = parse_kamino_limit_instruction(&instruction.data);
    ...
    fn get_config(&self) -> Option<&dyn SolanaIntegrationConfig> {
        Some(&KAMINO_LIMIT_CONFIG)
    }

    fn kind(&self) -> VisualizerKind {
        VisualizerKind::Dex(KAMINO_LIMIT_DISPLAY_NAME)
    }
}

/// Load and cache the bundled Kamino Limit Orders IDL.
fn load_kamino_limit_idl() -> Option<&'static Idl> {
    static IDL: std::sync::OnceLock<Option<Idl>> = std::sync::OnceLock::new();
    IDL.get_or_init(|| decode_idl_data(KAMINO_LIMIT_IDL_JSON).ok())
        .as_ref()
}

/// Parse an instruction's data using the bundled IDL.
fn parse_kamino_limit_instruction(
    data: &[u8],
    ...
    let idl = load_kamino_limit_idl().ok_or_else(|| {
        VisualSignError::DecodeError("failed to load Kamino Limit Orders IDL".into())
    })?;

    parse_instruction_with_idl(data, KAMINO_LIMIT_PROGRAM_ID, idl)
...
fn build_condensed_fields(
    ...
    Ok(vec![
        create_text_field("Program", KAMINO_LIMIT_DISPLAY_NAME)?,
...
mod tests {
    ...
    #[test]
    fn test_kamino_limit_idl_loads() {
        let idl = load_kamino_limit_idl().expect("IDL should load");
        assert!(
    ...
    #[test]
    fn test_kamino_limit_idl_has_discriminators() {
        let idl = load_kamino_limit_idl().expect("IDL should load");
    ...
    fn test_unknown_discriminator_returns_error() {
        ...
        let result = parse_kamino_limit_instruction(&garbage);
        assert!(
            result.is_err(),
    ...
    fn test_short_data_returns_error() {
        ...
        let result = parse_kamino_limit_instruction(&too_short);
        assert!(
            result.is_err(),
