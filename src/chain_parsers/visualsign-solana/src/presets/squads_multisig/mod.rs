...
use crate::core::{
    ...
    VisualizerKind, available_visualizers, format_arg_value, visualize_with_any,
};
...
impl InstructionVisualizer for SquadsMultisigVisualizer {
    fn visualize_tx_commands(
        ...
        // Resolve program_id + accounts against the transaction's account_keys, mirroring
        // the reference IDL preset (dflow_aggregator). An unresolved (ALT) index errors
        // here, so the framework falls back to the catch-all visualizer.
        ...
        let parsed = parse_squads_instruction(data, &accounts);

        let (title, condensed_fields, expanded_fields) = match parsed {
            Ok(parsed) => build_parsed_fields(&parsed, &program_id_str, context)?,
            Err(_) => build_fallback_fields(&program_id_str)?,
        };
...
fn try_build_vault_transaction_fields(
    ...
    // Visualize inner instructions using the full visualizer framework; each child
    // context is built at parent depth + 1 so re-entry is bounded at MAX_CALL_DEPTH.
    let inner_fields =
...
///
/// Recursion is bounded by the context's `call_depth`: once the parent context reaches
/// `MAX_CALL_DEPTH`, we return a single explicit "max depth exceeded" field rather than
/// recursing further. This protects against unbounded-recursion DoS via cyclically
/// nested `vaultTransactionCreate` payloads. Each child context is built at
/// `parent.call_depth() + 1` (saturating) so a re-entry into this visualizer is bounded.
fn visualize_inner_instructions(
    inner_instructions: &[Instruction],
    context: &VisualizerContext,
    inner_sender_override: Option<&SolanaAccount>,
) -> Result<Vec<AnnotatedPayloadField>, VisualSignError> {
    // Bound recursion at the trait boundary, mirroring swig_wallet: if the parent is
    // already at the cap, descending one more level would exceed it.
    ...
    // Saturating add so a maliciously deep parent depth can't wrap to 0 and slip past
    // the MAX_CALL_DEPTH guard on re-entry.
    let child_depth = context.call_depth().saturating_add(1);
    let idl_registry = context.idl_registry();

    let mut fields = Vec::with_capacity(inner_instructions.len());

    for (idx, instruction) in inner_instructions.iter().enumerate() {
        // Rebuild a CompiledInstruction + account_keys from the resolved Instruction:
        // program_id at index 0, then each account pubkey. Mirrors swig_wallet's
        // inner-instruction reconstruction so the standard visualizer framework applies.
        let mut account_keys = vec![instruction.program_id];
        for meta in &instruction.accounts {
            account_keys.push(meta.pubkey);
        }
        let Ok(num_accounts) = u8::try_from(instruction.accounts.len()) else {
            fields.push(create_text_field(
                &format!("Inner Instruction {}", idx + 1),
                "<unrenderable: too many accounts to index>",
            )?);
            continue;
        };
        ...
        let inner_context =
            VisualizerContext::new(&owned_sender, &compiled, &account_keys, idl_registry, idx)
                .with_call_depth(child_depth);
...
mod tests {
    ...
    fn test_visualize_inner_instructions_truncates_when_at_max_depth() -> TestResult {
        // Build a context already at MAX_CALL_DEPTH so visualize_inner_instructions
        // emits the explicit truncation field instead of descending another level.
        let sender = SolanaAccount {
            account_key: make_pubkey(9).to_string(),
            signer: false,
            writable: false,
        };
        let account_keys = vec![make_pubkey(1)];
        let compiled = solana_sdk::instruction::CompiledInstruction {
            program_id_index: 0,
            accounts: vec![],
            data: vec![],
        ...
        let current = VisualizerContext::new(&sender, &compiled, &account_keys, &registry, 0)
            .with_call_depth(MAX_CALL_DEPTH);
        // current.call_depth() == MAX_CALL_DEPTH; visualize_inner_instructions must
        // refuse to descend and emit the truncation message.
