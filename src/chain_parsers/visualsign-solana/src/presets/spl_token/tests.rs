...
#[test]
fn test_revoke_visualization_falls_through_to_default() {
    // Revoke has no dedicated branch — exercises the fallback layout.
    let source = Pubkey::new_unique();
    let owner = Pubkey::new_unique();
    ...
    run_visualization_test(
        instruction,
        "Revoke",
        &["Instruction", "Program", "Raw Data"],
    );
}

#[test]
fn test_close_account_visualization_falls_through_to_default() {
    let account = Pubkey::new_unique();
...
#[test]
fn test_initialize_mint_visualization_falls_through_to_default() {
    let mint = Pubkey::new_unique();
    let mint_authority = Pubkey::new_unique();
    let instruction =
