...
- Surfpool-backed integration tests (`visualsign-solana/tests/surfpool_fuzz.rs`) are `#[ignore]` and require the `surfpool` binary on `$PATH`. The `run_idl_roundtrip`/`run_idl_roundtrip_inner` bodies and the `#[macro_export]`ed `idl_test!` macro live in `tests/common/mod.rs`, reusable from any test file that does `mod common;`. Run with `HELIUS_API_KEY=<key> cargo test -p visualsign-solana --test surfpool_fuzz -- --ignored --test-threads=1`.
  - Upstream IDLs: each `idl_test!(...)` invocation in `surfpool_fuzz.rs` references a `pub const` from `solana_parser::solana::embedded_idls`, so IDL contents are baked in at compile time and the macro expands into one `#[tokio::test]` discoverable by cargo. Adding a new IDL: once it's exposed as a `pub const` upstream in `solana_parser`, add an `idl_test!(name, CONST)` line. `idl_tests_cover_all_program_types` is a plain (non-`#[ignore]`) test that fails if this list drifts from `solana_parser::ProgramType::all()`, so a rev bump that adds or removes a program type is caught on every `cargo test`, not just surfpool-labeled CI runs.
  - Vsp-local preset IDLs: `build.rs` reflects `src/presets/<name>/<name>.json` into `pub const PRESET_IDLS: &[(&str, &str)]`; `surfpool_preset_idls` iterates it automatically, so a preset added via the `solana-add-idl` skill needs no test-file edit.
  - To validate a candidate IDL before it's embedded upstream, set `IDL_FILE=<path>` and run `surfpool_idl_from_env` — it skips gracefully when unset.
